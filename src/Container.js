...
export default class Container {
    ...
    /**
     * (method) Get an Instances Constructor Name
     * @param obj {*}
     * @return {String}
     */
    getName(obj){
        return  obj ? obj.name || obj.constructor.name || null : null
    }
    ...
    listBindings() {
        ...
        console.group('Container Bindings:')
        console.table(Object.keys(this._bindings))
        console.log(this._bindings)
        console.groupEnd()
    }

    /**
     * (method) List Registered Providers
     * @return void
     */
    listProviders() {
        if(!this._debug) return
        console.group('Container Services:')
        console.table(this._providers, ['name', '_deferred', '_booted'])
        console.groupEnd()
    }

    /**
     * (method) List Sharable Services
     * @return void
     */
    listSharable() {
        if(!this._debug) return
        console.group('Container Shared Instances:')
        console.table(Object.keys(this._bindings), this._sharable)
        console.groupEnd()
    }
     ...
     */
    handleError(error) {
        if (this._errorHandler && this._isCallable(this._errorHandler.handle)) {
     ...
     */
    isRegistered(alias) {
        return this._isset(this._providers[alias])
    }
    ...
    destroy(alias) {
        if (this.isResolved(alias)) {
            ...
            if (this._isCallable(this._resolved[alias].$destroy)) {
    ...
    _resolveIfNotResolved(alias) {
        ...
        const dependencies = this._prepareForInjection(alias, binding)
        return this._makeConcrete(binding, dependencies)
    }
    ...
    _makeConcrete(binding, injections){
        try {
            if (!this._isCallable(binding)) {
                if (injections.length) throw this.makeException(
                    'Binding Exception',
                    `Cannot pass dependencies to non-callable binding ${binding}.`
                )
                return binding
            }
            const concrete = new (Object.create(Function.prototype).bind.apply(binding, [null].concat(injections)))

            if(typeof concrete === 'undefined'){
                return this.makeException('Binding Exception', `Binding ${binding} failed, return value is undefined.`)
            }

            this.log(`Instantiated Concrete Instance successfully. ${this.getName(concrete)}`, concrete)
